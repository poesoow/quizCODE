    ...
    <div class="mx-auto w-10/12 lg:w-6/12 flex flex-wrap items-center">
      <form @submit.prevent>
        <div class="w-full bg-white rounded-lg p-5 flex gap-x-5 flex-wrap justify-center">
          ...
          <div class=" mt-4 text-xs sm:text-sm font-bold"><span v-if="userName != ''" class="block mb-2">{{ userName }}님 반갑습니다.</span> 문제 유형은 {{ selectRandom === '0' ? '기본값' : '랜덤' }}이고, {{ selectType.toUpperCase() }}유형을 선택하였으며, 총 {{ cateList.length }}개의 문제 중 {{ selectCount }}문제를 선택하였습니다.</div>
        </div>
      </form>
